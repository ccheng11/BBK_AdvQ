...
In this section, we will use the workhorse function `glm()` to estimate the correlates of people's attitudes toward abortion.
  ...
  - Fit the logit model with one predictor `religion` (see below).
...
## 3.3 Logit Regression with One Predictor
...
### Option 2: Exponentiate $\hat{\beta}$ to Get Odds Ratio (OR)
...
A rule of thumb for odds-ratio (OR):

  - If $OR=1$, it means $\text{Odds}_1$ and $\text{Odds}_0$ are the same, so changing $X$ from 0 to 1 does not have much impact on the outcome (i.e., $X$ is not associated with the odds of the outcome).
  - If $OR>1$, it means $\text{Odds}_1$ is larger $\text{Odds}_0$ so that changing $X$ from 0 to 1 will make the outcome of interest more likely to happen (i.e., $X$ is positively associated with the odds of the outcome).
  - If $OR<1$, it means $\text{Odds}_1$ is small $\text{Odds}_0$ so that changing $X$ from 0 to 1 will make the outcome of interest less likely to happen (i.e., $X$ is negatively associated with the odds of the outcome).
