...
For the purpose of demonstration, we will focus on the following variables in this tutorial.
  ...
  - `happy` (ordinal): Perceptions of corruption in the country; the variable ranges from 1 to 4; "1" means *very happy* and "4" means *not at all happy*. 
...
```{r, echo=T, eval=T, message=F}
dta_new <- dta %>%
  select(Q46P, Q275, regtype) %>%
  rename(happy = Q46P,
         edu = Q275) %>%
  mutate(happy = ifelse(happy <= 0, NA, happy),
         edu = ifelse(edu <= 0, NA, edu),
         regtype = ifelse(regtype <= 0, NA, regtype)) %>%
  mutate(regtype = as.character(regtype)) %>%
  mutate(reg_type = recode(regtype,
                           "5" = "full democracy",
                           "4" = "democracy",
                           "3" = "open anocracy",
                           "2" = "closed anocracy",
                           "1" = "autocracy")) %>%
  tidyr::drop_na(regtype)
```
...
Let's check if the recoding is done properly.
...
```{r, echo=T, eval=T, message=F}
dta_autocracy <- dta_new %>% filter(regtype == 1)
...
```{r, echo=T, eval=T, message=F}
dta_democracy <- dta_new %>% filter(regtype == 5)
...
There seems to be a positive correlation between `edu` and `happy`. 
...
```{r, echo=T, eval=F, message=F, warning=F}
options(scipen=999)
mod_lmm_int <- lmer(happy ~ edu + (0+edu|reg_type), data=dta_new)
```
...
```{r, echo=T, eval=F, message=F, warning=F}
options(scipen=999)
mod_lmm_int <- lmer(happy ~ edu + (1+edu|reg_type), data=dta_new)
```
