...
If you have yet to install the packages, use the `install.packages()` function.
...
> *Question: Can you use a few words to describe what each of these packages does?*

### **2 Data**

The sample dataset for today's tutorial is from the 2011 <a href="http://www.ces-eec.ca/?target=_blank" target="_blank">Canadian National Election Study</a>. We will use the data stored in the `carData` package. You can also find the csv file on **Moodle**.
...
> *Question: When should we use stratified sampling? And how does that influence the distribution of respondents from different provinces when we do not use stratified sample?*\

> *Question: What is the main dependent variable of interest?*\

> *Question: Anything wrong with the current set of explanatory/independent variables or predictors here? What factors do we leave out here?*

> *Question: Can we conduct survey research with children in our sample?*

Now, set up your working directory and read the data.

```{r, echo=T, eval=F}
ces <- read.csv("ces11.csv", stringsAsFactors=TRUE)
```

Here we set `stringsAsFactors=TRUE` so that the variables that are meant to be factors are set up accordingly.\
